...
> SimpleTarget后面的泛型一般可以是 Bitmap，GlideDrawable，GifDrawable，这个和load方法之后的处理有关，如果确认是bitmap调用asBitmap（）方法的话SimpleTarget的泛型为Bitmap，如果调用asGif（）则泛型为GifDrawable，默认情况下则为GlideDrawable。
