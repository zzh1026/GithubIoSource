...
- get的每个方法会对调用线程进行区分，如果不在主线程则所有的get方法都会调用get(@NonNull Context context)从而调用到getApplicationManager(@NonNull Context context)，这个方法返回的周期为Application
